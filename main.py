...
def tokenz():
	...
	try:
		...
		print(" [*] Github     : https://github.com/Gak punya:(")
		print(" [*] ---------------------------------------------")
...
def menu():
    ...
    if asw == "":
    	...
    	os.system('rm -f token.txt')
    	jalan(" [鉁揮 berhasil menghapus token ")
    	exit()
    else:
...
def check_in(user, pasw):
	...
	if "c_user" in ses.cookies:
		...
		if "Lihat detail login yang ditampilkan. Ini Anda?" in str(xnxx):
			print("\r  馃専 %sTinggal 1 langkah lagi untuk membuka akun facebook. silahkan buka di browser%s"%(H,N))
		else:
...
def sandimanual():
	...
	print("\n [ pilih method version - silahkan coba satu虏 ]\n")
